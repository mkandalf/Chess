# To Do: Check detection, pawn promotion, sliding pieces, captured pieces
import constants
import util
...
def gen_moves(board):
  ...
  gen_pawn_moves(board)
  gen_pawn_captures(board)
...
def gen_knight_moves(board):
  ...
  while knights:
    ...
    while (to):
      move_list.append(
        gen_move(util.bit_scan_forward(to), loc, 1, 0))
      to = util.clear_least_bit(to)
  return move_list
