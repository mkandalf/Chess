...
from bitboard import shift, print_bb, flip_diag_A1H8


PAWN_ADANCE_2_MASK = [uint64(0xff000000), uint64(0xff00000000)]
RANK_MASK = [
    uint64(0xff),
    uint64(0xff00),
    uint64(0xff0000),
    uint64(0xff000000),
    uint64(0xff00000000),
    uint64(0xff0000000000),
    uint64(0xff000000000000),
...
#TODO: Redefine magic numbers
#TODO: Add doctest
def gen_rook_occ(sq):
  sq_rank, sq_file = sq >> 3, sq & 7
  aaa = 2 ** max(0, 6 - sq_file)
  bbb = 2 ** max(0, sq_file - 1)
  ccc = uint64(8 * sq_rank + sq_file + 1)
  for i in xrange(aaa):
    ret_left = uint64(i) << ccc
    for j in xrange(bbb):
      ...
      for k in xrange(bbb):
        ...
        for l in xrange(aaa):
          ret_bottom = ret_top | flip_diag_A1H8(uint64(l) << ccc)
          yield uint64(ret_bottom)


def knight_attacks(bb_loc):
    """Generate the bitboard for possible knight attacks at a square.
    >>> bb_loc = uint64(2 ** 28)
    >>> print_bb(bb_loc)
    00000000
    00000000
    ...
    00000000
    00000000
    >>> print_bb(knight_attacks(bb_loc))
    00000000
    00000000
    00101000
    01000100
    ...
    00000000
    """
    assert type(bb_loc) == uint64
    east = shift(bb_loc, 1, 0)
    west = shift(bb_loc, -1, 0)
...
def king_attacks(bb_loc):
    """Generate the possible bb_loc attack at a square.
    >>> bb_loc = uint64(2 ** 28)
    >>> print_bb(bb_loc)
    00000000
    00000000
    ...
    """
    assert type(bb_loc) == uint64
    bb_attacks = shift(bb_loc, 1, 0) | shift(bb_loc, -1, 0)
    bb_loc |= bb_attacks
    bb_attacks |= (shift(bb_loc, 0, 1) | shift(bb_loc, 0, -1))
    return bb_attacks


try:
  with open('KNIGHT_ATTACKS.p', 'r') as f:
    KNIGHT_ATTACKS = cPickle.load(f)
except IOError:
  KNIGHT_ATTACKS = [knight_attacks(uint64(sq)) for sq in xrange(64)]
  with open('KNIGHT_ATTACKS.p', 'w') as f:
    cPickle.dump(KNIGHT_ATTACKS, f)


try:
  with open('KING_ATTACKS.p', 'r') as f:
    KING_ATTACKS = cPickle.load(f)
except IOError:
  KING_ATTACKS = [king_attacks(uint64(sq)) for sq in xrange(64)]
  with open('KING_ATTACKS.p', 'w') as f:
    cPickle.dump(KING_ATTACKS, f)


try:
  with open('ROOK_OCC.p', 'r') as f:
    ROOK_OCC = cPickle.load(f)
except IOError:
  ROOK_OCC = [[o for o in gen_rook_occ(sq)] for sq in xrange(64)]
  with open('ROOK_OCC.p', 'w') as f:
    cPickle.dump(ROOK_OCC, f)


#TODO: Break this into a function like the others
#TODO: Add a doctest
try:
  with open('ROOK_MOVES.p', 'r') as f:
    ROOK_MOVES = cPickle.load(f)
except IOError:
  ...
  with open('ROOK_MOVES.p', 'w') as f:
    cPickle.dump(ROOK_MOVES, f)
